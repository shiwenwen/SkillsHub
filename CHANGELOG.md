...
- 一次性补齐 `de/es/fr/ja/ko/pt/ru` 语言包中 `installed` 分组缺失字段，修复 `Translations` 类型不匹配导致的 `tsc` 构建失败

### Changed
- 设置页面「关于」标签页 UI 重构为深色渐变卡片风格，新增作者/开源入口信息卡、技术栈标签与更新按钮，并移除“公众号”和“支持作者”相关区块
