import type { Translations } from "./en";

// 中文翻译
export const zh: Translations = {
    // 通用
    common: {
        version: "版本",
        save: "保存",
        cancel: "取消",
        close: "关闭",
        delete: "删除",
        install: "安装",
        installed: "已安装",
        installing: "安装中...",
        uninstall: "卸载",
        sync: "同步",
        syncing: "同步中...",
        refresh: "刷新",
        search: "搜索",
        loading: "加载中...",
        noDescription: "暂无描述",
        detected: "已检测",
        notFound: "未找到",
        popular: "热门",
        name: "名称",
        size: "大小",
        noData: "暂无数据",
        error: "错误",
        add: "添加",
        about: "关于",
        scan: "扫描",
        allSources: "所有来源",
    },

    // 导航
    nav: {
        installed: "已安装",
        discover: "发现",
        sync: "同步",
        security: "安全",
        settings: "设置",
        appDescription: "Agent Skills 管理器",
        unifiedSkillsManagement: "统一 Skills 管理",
    },

    // 已安装页面
    installed: {
        title: "已安装的 Skills",
        description: "管理已安装的 Agent Skills",
        installNew: "安装新 Skills",
        totalSkills: "Skills 总数",
        scanPassed: "扫描通过",
        synced: "已同步",
        noSkills: "暂无已安装 Skills",
        noSkillsHint: "开始发现并安装 Skills",
        discoverSkills: "发现 Skills",
        viewDetails: "查看详情",
        secure: "安全",
        issues: "问题",
        checkUpdates: "检查更新",
        checkingUpdates: "检查中...",
        updatesAvailable: "有可用更新",
        skillsCanUpdate: "个 Skills 可以更新",
        syncing: "同步中...",
        fullSync: "完整同步",
        searchPlaceholder: "搜索 Skills...",
        hubStorage: "Hub 存储",
        localScanned: "本地扫描",
        plugins: "插件",
        active: "活跃",
        hubView: "Hub 视图",
        scannedView: "扫描视图",
        pluginsView: "插件视图",
        syncAll: "全部同步",
        skillName: "Skill 名称",
        details: "详情",
        status: "状态",
        actions: "操作",
        missingIn: "缺失于",
        fullySynced: "已完全同步",
        inHub: "在 Hub 中",
        notInHub: "不在 Hub 中",
        noSkillsInHub: "Hub 中未找到 Skill",
        noScannedSkills: "未找到扫描到的 Skill",
        noPlugins: "未找到已安装的插件",
        fullSyncing: "正在执行完整同步...",
        fullSyncSuccess: "同步完成: 收集 {collected} 个, 分发 {distributed} 次",
        fullSyncFailed: "同步失败: {error}",
        view: "查看",
        loadFailed: "加载失败：{error}",
    },

    // 发现页面
    discover: {
        title: "发现 Skills",
        description: "从仓库中查找和安装 Skills",
        source: "来源",
        searchPlaceholder: "搜索 Skills...",
        searchForSkills: "搜索 Skills",
        searchHint: "输入搜索内容或按标签浏览",
        featuredSkills: "精选 Skills",
        curatedSkills: "社区精选 Skills",
        by: "作者",
        unknown: "未知",
        installBlocked: "安装已阻止：Skill \"{skillId}\" 被检测为高风险。",
        installFailed: "安装失败：{error}",
        loadingMore: "加载更多 Skills...",
        endOfResults: "已到末尾",
    },

    // 同步页面
    syncDashboard: {
        title: "同步面板",
        description: "管理 Skills 在各工具间的同步",
        lastSync: "上次同步",
        never: "从未",
        syncAll: "全部同步",
        detectedTools: "检测到的工具",
        skillsActive: "个 Skills 已激活",
        open: "打开",
        syncHistory: "同步历史",
        synced: "已同步",
        to: "到",
        justNow: "刚刚",
        hourAgo: "小时前",
        hoursAgo: "小时前",
        active: "活跃",
        notFound: "未找到",
        skillsSynced: "个 Skills 已同步",
        driftDetection: "差异检测",
        allInSync: "所有 Skills 已同步，未检测到差异。",
        tool: "工具",
        issue: "问题",
        actions: "操作",
        repair: "修复",
        syncStrategy: "同步策略",
        linkRecommended: "链接（推荐）",
        linkDesc: "符号链接到共享源。即时更新，节省空间。",
        copy: "复制",
        copyDesc: "完整文件复制。更好的兼容性，更多存储。",
        pathNotFound: "未找到路径",
        notDetectedInStandard: "未在标准位置检测到",
        builtIn: "内置",
        status: "状态",
        syncedSkills: "已同步 Skills",
        pathCount: "{count} 个路径",
        relativeFromProjectRoot: "（相对于项目根目录）",
        noPathsConfigured: "此工具未配置路径",
    },

    // 安全页面
    security: {
        title: "安全中心",
        description: "管理安全扫描和策略",
        scanAll: "扫描所有 Skills",
        noIssues: "无问题",
        riskLevel: "风险等级",
        passed: "已通过",
        lowRisk: "低",
        mediumRisk: "中",
        highRisk: "高",
        permissions: "权限",
        readFiles: "读取文件",
        writeFiles: "写入文件",
        networkAccess: "网络访问",
        executeCommands: "执行命令",
        lastScanned: "上次扫描",
        viewReport: "查看报告",
        securitySettings: "安全设置",
        scanBeforeInstall: "安装前扫描",
        scanBeforeUpdate: "更新前扫描",
        blockHighRisk: "默认阻止高风险",
        scanned: "已扫描",
        skillsVerified: "Skills 已验证",
        warnings: "警告",
        requireReview: "需要审核",
        blocked: "已阻止",
        criticalIssues: "严重问题",
        activeSecurityRules: "活动安全规则",
        ruleId: "规则 ID",
        name: "名称",
        status: "状态",
        enabled: "已启用",
        scanPolicy: "扫描策略",
        blockHighRiskFindings: "阻止高风险发现",
        requireConfirmMedium: "中等风险需确认",
        autoApproveLow: "自动批准低风险",
        trustedSources: "可信来源",
        remove: "移除",
        addTrustedSource: "添加可信来源",
        addRegistryAsTrustedSource: "添加仓库来源",
        noConfiguredRegistries: "没有已配置仓库",
        recentScanResults: "最近扫描结果",
        skill: "Skill",
        scannedAt: "扫描时间",
        findings: "发现",
    },

    // 设置页面
    settings: {
        title: "设置",
        description: "配置 SkillsHub 偏好",
        general: "通用",
        defaultSyncStrategy: "默认同步策略",
        autoLinkFirst: "自动（优先链接）",
        alwaysLink: "始终链接",
        alwaysCopy: "始终复制",
        toolSyncStrategy: "同步策略",
        useGlobalStrategy: "使用全局默认",
        autoSyncOnInstall: "安装时自动同步",
        checkUpdatesOnStartup: "启动时检查更新",
        storage: "存储",
        openDirectory: "打开目录",
        localStoreLocation: "本地存储位置",
        storageUsed: "已用空间",
        skillsStored: "存储 Skills 数",
        registries: "仓库",
        addRegistry: "添加仓库",
        toolConfiguration: "工具配置",
        toolConfigDescription: "配置每个 AI 编码工具的 skills 路径。点击展开编辑路径。",
        globalPath: "全局路径",
        projectPath: "项目路径",
        noGlobalPath: "无全局路径（仅支持项目级别）",
        addCustomTool: "添加自定义工具",
        customTool: "自定义",
        customTools: "自定义工具",
        toolName: "工具名称",
        toolNamePlaceholder: "我的自定义工具",
        globalPathHint: "如果此工具仅支持项目级别的 skills，可留空",
        projectPathHint: "相对于项目根目录的路径",
        addTool: "添加工具",
        selectDirectory: "选择目录",
        security: "安全",
        scanBeforeInstall: "安装前扫描",
        scanBeforeUpdate: "更新前扫描",
        blockHighRisk: "默认阻止高风险",
        saveSettings: "保存设置",
        saveSuccess: "设置已保存",
        saveFailed: "保存设置失败",
        noChangesToSave: "没有需要保存的更改",
        validationTitle: "设置校验失败，无法保存：",
        invalidSyncStrategy: "同步策略无效",
        cloudProviderRequired: "请选择云存储提供商",
        cloudFolderRequired: "云同步文件夹不能为空",
        invalidCloudProvider: "云存储提供商无效",
        invalidPathChars: "路径包含非法字符",
        invalidCustomToolName: "自定义工具名称不能为空",
        invalidCustomToolProjectPath: "自定义工具项目路径必须为相对路径",
        language: "语言",
        languageDescription: "选择界面语言",
        chinese: "中文",
        english: "English",
        cloudSync: "云端同步",
        cloudSyncDescription: "通过云存储跨设备同步技能",
        cloudProvider: "云存储提供商",
        selectProvider: "选择云存储提供商",
        customFolder: "自定义文件夹",
        cloudSyncFolder: "同步文件夹",
        cloudAutoSync: "变更时自动同步到云端",
        syncNow: "立即同步",
        syncing: "同步中...",
        lastCloudSync: "上次云端同步",
        noCloudDrives: "未检测到云端驱动器",
        cloudSyncSuccess: "云端同步完成",
        skillsPushed: "个技能已推送",
        skillsPulled: "个技能已拉取",
        theme: "主题",
        themeDescription: "选择你喜欢的配色方案",
        themeAuto: "自动",
        themeLight: "日间",
        themeDark: "夜间",
        themeAutoDescription: "跟随系统设置",
        themeLightDescription: "亮色配色方案",
        themeDarkDescription: "暗色配色方案",
        aboutSubtitle: "统一 AI Skills 管理",
        aboutVersion: "v1.0.5",
        aboutAuthorLabel: "作者",
        aboutOpenSourceLabel: "开源地址",
        aboutOpenSourceAction: "查看代码",
        aboutCheckUpdate: "检查更新",
        aboutCopyright: "© 2026 SkillsHub. 版权所有。",
        scanBeforeInstallDesc: "在添加到库之前扫描代码",
        scanBeforeUpdateDesc: "在更新之前扫描代码",
        blockHighRiskDesc: "自动阻止高风险操作",
        noSyncHistory: "暂无同步记录",
        registryName: "名称",
        registryNamePlaceholder: "my-registry",
        registryUrl: "Git URL",
        registryUrlPlaceholder: "https://github.com/user/repo",
        registryBranch: "分支（可选）",
        registryBranchPlaceholder: "main",
        registryDescription: "描述（可选）",
        registryDescriptionPlaceholder: "描述...",
        editCustomTool: "编辑自定义工具",
        updateChecking: "正在检查更新...",
        updateUpToDate: "当前已是最新版本",
        updateCheckAgain: "重新检查",
        updateAvailable: "发现新版本",
        updateReleaseNotes: "更新日志",
        updateDownloadInstall: "下载并安装",
        updateDownloading: "正在下载更新...",
        updateReadyRestart: "更新就绪，重启以应用",
        updateRestartNow: "立即重启",
        updateError: "检查更新失败",
        updateRetry: "重试",
    },

    // 技能详情页面
    skillDetail: {
        back: "返回已安装",
        by: "作者",
        installedAt: "安装时间",
        source: "来源",
        update: "更新",
        readme: "说明文档",
        files: "文件",
        permissions: "权限",
        syncStatus: "同步状态",
        syncToAll: "同步到所有",
        synced: "已同步",
        notSynced: "未同步",
        noReadme: "该 Skill 暂无说明文档。",
        file: "个文件",
        files_plural: "个文件",
        skillNotFound: "Skill 未找到",
        version: "版本",
        secure: "安全",
        issuesFound: "发现问题",
        details: "详情",
        installed: "已安装",
        syncedTools: "已同步工具",
        notSyncedToAny: "尚未同步到任何工具",
        overview: "概览",
        security: "安全",
        securityScanResults: "安全扫描结果",
        passedAllChecks: "该 Skill 已通过所有安全检查",
        tool: "工具",
        status: "状态",
        strategy: "策略",
        actions: "操作",
        autoLink: "自动（链接）",
        securityStatus: "安全状态",
        passed: "已通过",
        lastScanned: "上次扫描",
        viewFullReport: "查看完整报告",
        loadingSkillDetails: "正在加载 Skill 详情...",
        skillNotFoundDesc: "未找到您要查找的 Skill。",
        issuesDetected: "发现问题",
        skillSafeToUse: "根据当前安全规则，此 Skill 可以安全使用。",
        potentialIssuesFound: "发现 {count} 个潜在问题。风险等级：{risk}",
        detailedFindings: "详细发现",
        copyStrategy: "复制",
    },
};
