...
use crate::registry::{
    calculate_dir_hash, RegistryProvider, SkillListing, SkillQuery,
};
...
impl ClawHubRegistry {
    ...
    async fn download_file(&self, url: &str, dest: &PathBuf) -> Result<()> {
        let response = self.client.get(url).send().await.map_err(|e| Error::System(format!("Failed to download file: {}", e)))?;
        
        if !response.status().is_success() {
            return Err(Error::System(format!("Failed to download file: status {}", response.status())));
        }

        let content = response.bytes().await.map_err(|e| Error::System(format!("Failed to read content: {}", e)))?;
...
impl RegistryProvider for ClawHubRegistry {
    ...
    async fn search(&self, query: &SkillQuery) -> Result<Vec<SkillListing>> {
        ...
        if !response.status().is_success() {
             return Err(Error::System(format!("ClawHub API error: {}", response.status())));
        }

        let data: ClawHubSearchResponse = response.json().await
             .map_err(|e| Error::System(format!("Failed to parse ClawHub response: {}", e)))?;

        let listings = data.items.into_iter().map(|item| {
            SkillListing {
                id: item.slug.clone(),
    ...
    async fn fetch(&self, skill_id: &str, dest: &std::path::Path) -> Result<PathBuf> {
        ...
        if !output.status.success() {
             return Err(Error::System(format!("Unzip failed: {}", String::from_utf8_lossy(&output.stderr))));
        }
